...
describe('ExampleToken', function () {
  ...
  const TOTAL_SUPPLY = 1000000n * EXT;
  ...
  const ProtectionConfig = {
    ...
    LiquidityAmountTrap_blocks: 3,
    LiquidityAmountTrap_amount: 2000n * EXT,

    LiquidityPercentTrap_skip: false,
    LiquidityPercentTrap_blocks: 7,
    LiquidityPercentTrap_percent: HUNDRED_PERCENT / 10n,

    LiquidityActivityTrap_skip: false,
    LiquidityActivityTrap_blocks: 10,
    LiquidityActivityTrap_count: 15,

    TokensToPutIntoLiquidityPool: 100000n * EXT,
    PredeployedLPS: '0x',
  };
  ...
  it('Should have valid initial distribution', async function () {
    ...
    await expect(ext.connect(user1Signer).revokeBlocked([user1, user2, user3], revoker)).to.be.revertedWith(
      'UsingLiquidityProtectionService: not admin'
    );
  });
  ...
  it('Should trap all buyers in the first block', async function () {
    ...
    await mine();
    await startMining();
    await expect(ext.connect(user1Signer).transfer(user3, 1)).to.be.revertedWith('FirstBlockTrap: blocked');
  ...
  it('Should trap all buyers who bought above amount limit', async function () {
    ...
    await expect(ext.connect(user2Signer).transfer(user1, 1)).to.be.revertedWith('FirstBlockTrap: blocked');
    await expect(ext.connect(user3Signer).transfer(user1, 1)).to.be.revertedWith('FirstBlockTrap: blocked');
  ...
  it('Should trap all buyers who bought above percent limit', async function () {
    ...
    await addToBlock(() => ext.transfer(user3, 2, GAS_LIMIT));
    await addToBlock(() =>
      ext.connect(lpSigner).transfer(user1, (liquidity * PERCENT_TRAP) / HUNDRED_PERCENT - 2n, GAS_LIMIT)
    );
    liquidity -= (liquidity * PERCENT_TRAP) / HUNDRED_PERCENT - 2n;
    await addToBlock(() =>
      ext.connect(lpSigner).transfer(user2, (liquidity * PERCENT_TRAP) / HUNDRED_PERCENT - 1n, GAS_LIMIT)
    );
    liquidity -= (liquidity * PERCENT_TRAP) / HUNDRED_PERCENT - 1n;
    // Block user3.
    const tx1 = await addToBlock(() =>
      ext.connect(lpSigner).transfer(user3, (liquidity * PERCENT_TRAP) / HUNDRED_PERCENT + 1n, GAS_LIMIT)
    );
    liquidity -= (liquidity * PERCENT_TRAP) / HUNDRED_PERCENT + 1n;
    await addToBlock(() => ext.connect(lpSigner).transfer(user3, 1, GAS_LIMIT));
    liquidity -= 1n;
  ...
  it('Should trap all traders if more than 15 trades in the second block', async function () {
    ...
    await expect(ext.connect(user2Signer).transfer(user1, 1)).to.be.revertedWith('LiquidityActivityTrap: blocked');
    await expect(ext.connect(user1Signer).transfer(owner, 1)).to.be.revertedWith('LiquidityActivityTrap: blocked');
    ...
    await ext.revokeBlocked([user1, user2, user3], revoker);
    expect(await ext.balanceOf(revoker)).to.equal(227 + 6);
    expect(await ext.balanceOf(user1)).to.equal(0);
    expect(await ext.balanceOf(user2)).to.equal(0);
    expect(await ext.balanceOf(user3)).to.equal(5);
  });

  it('Should trap all traders if more than 15 trades in the third block, but do not trap second block traders', async function () {
    if (ProtectionConfig.LiquidityActivityTrap_skip) {
      console.log('LiquidityActivityTrap disabled.');
      return;
    }
  ...
  it('Should disable protection', async function () {
    ...
    await startMining();
    await expect(ext.connect(user1Signer).transfer(user3, 1)).to.be.revertedWith('FirstBlockTrap: blocked');
    await expect(ext.connect(user1Signer).disableProtection()).to.be.revertedWith(
      'UsingLiquidityProtectionService: not admin'
    );
    await ext.disableProtection();
    ...
    await ext.connect(lpSigner).transfer(user3, 1);
    await ext.connect(user3Signer).transfer(lp, 1);
    await expect(ext.revokeBlocked([user1, user2, user3], revoker)).to.be.revertedWith(
