...
abstract contract UsingLiquidityProtectionService {
    ...
    function LPS_beforeTokenTransfer(
        address _from,
        address _to,
        uint256 _amount
    ) internal {
        if (protectionChecker()) {
            ...
            require(
                FirstBlockTrap_skip() ||
                    lps().FirstBlockTrap_preValidateTransfer(
                        _from,
                        _to,
                        _amount,
                        counterToken()
                    ),
                "FirstBlockTrap: blocked"
            );
            require(
                LiquidityAmountTrap_skip() ||
                    lps().LiquidityAmountTrap_preValidateTransfer(
                        _from,
                        _to,
                        _amount,
                        counterToken(),
                        LiquidityAmountTrap_blocks(),
                        LiquidityAmountTrap_amount()
                    ),
                "LiquidityAmountTrap: blocked"
            );
            require(
                LiquidityPercentTrap_skip() ||
                    lps().LiquidityPercentTrap_preValidateTransfer(
                        _from,
                        _to,
                        _amount,
                        counterToken(),
                        LiquidityPercentTrap_blocks(),
                        LiquidityPercentTrap_percent()
                    ),
                "LiquidityPercentTrap: blocked"
            );
            require(
                LiquidityActivityTrap_skip() ||
                    lps().LiquidityActivityTrap_preValidateTransfer(
                        ...
                        LiquidityActivityTrap_count()
                    ),
                "LiquidityActivityTrap: blocked"
            );
        }
    }
